...
from .. import models
from database.schemas import usage_schema, keycloak_schema
...
def get_usages(
    ...
    #ins_user_id: str = "", 
    #ins_belongs_to_group: bool = False, 
    #ins_group_id: str = "", 
    #ins_cluster_id: int = 0, 
    ins_robot_type: str = "", 
    ins_id: int = 0, 
    skip: int = 0, 
    limit: int = 100,
    ...
    usages = usages.filter(
        models.Usage.is_terminated == is_terminated,
        models.Usage.ins_user_id == credentials.user_id
    )
...
def create_usage(
    ...
    if db_instance.user_id != credentials.user_id:
        raise HTTPException(
            status_code=403,
            detail="Cannot create a usage record of another user's instance"
        )
    ...
    db_usage = models.Usage(
        ...
        ins_release_name=db_instance.release_name,
...
def delete_usage(
    ...
    query_exec = db.query(models.Usage).filter(
        models.Usage.id == id,
        models.Usage.ins_user_id == credentials.user_id
    ).delete()
    db.commit()
